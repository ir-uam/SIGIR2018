...
/**
 * Optimal non-personalized recommender in terms of observed precision. 
 *
 ...
 */
public class OptimalObservedRanking<U,I> extends AbstractFastRecommender<U, I> {
     ...
     */
    public OptimalObservedRanking(FastPreferenceData<U, I> trainData, FastPreferenceData<U, I> testData, double threshold) {
        ...
        popList = trainData.getIidxWithPreferences()
                .mapToObj(iidx -> {
                    long relTest = testData.getIidxPreferences(iidx).filter(ip -> ip.v2 >= threshold).count();
                    long mTrain = trainData.numUsers(iidx);
                    return tuple(iidx, relTest * 1.0 /(m - mTrain));
                })
